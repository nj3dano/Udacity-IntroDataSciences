...
def get_hourly_entries(df):
    ...
    '''
    # create a new column using the ENTRIESn column shifted down 1
    df['ENTRIESn_hourly'] = df['ENTRIESn'].shift(1)
    
    # do the math
    df['ENTRIESn_hourly'] = df['ENTRIESn'] - df['ENTRIESn_hourly']
    
    # If there is any NaN, fill/replace it with 1
    # it wants the very first entry to be a 1, so replace after you do the math
    # if you replace first, you cannot tell the difference between a missing value
    # and a value that is actually 1.
